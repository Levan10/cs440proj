...
def get_users_with_only_poor_reviews():
    cursor.execute("""
        ...
        AND NOT EXISTS (
            SELECT 1
            FROM reviews rv2
            WHERE rv2.username = rv.username AND rv2.rating != 'poor'
        )
    """)
    results = cursor.fetchall()
    return [row[0] for row in results]

def get_users_with_units_without_poor_reviews():
    cursor.execute("""
        ...
        WHERE NOT EXISTS (
            SELECT 1
            FROM reviews rv
            WHERE rv.unit_id = r.unit_id AND rv.rating = 'poor'
        )
    """)
    results = cursor.fetchall()
    return [row[0] for row in results]
...
def on_get_users_with_only_poor_reviews():
    results = get_users_with_only_poor_reviews()
    user_results_box.delete("1.0", tk.END)
    if results:
        user_results_box.insert(tk.END, "\n".join(results))
    else:
        user_results_box.insert(tk.END, "No users found with only 'Poor' reviews.")

def on_get_users_with_units_without_poor_reviews():
    results = get_users_with_units_without_poor_reviews()
    user_results_box.delete("1.0", tk.END)
    if results:
        user_results_box.insert(tk.END, "\n".join(results))
    else:
        user_results_box.insert(tk.END, "No users found with units without 'Poor' reviews.")
...
tk.Button(root, text="Users with Only 'Poor' Reviews", command=on_get_users_with_only_poor_reviews).grid(row= 8, column=2, columnspan=2, pady=10)
tk.Button(root, text="Users with Units Without 'Poor' Reviews", command=on_get_users_with_units_without_poor_reviews).grid(row=9, column=2, columnspan=2, pady=10)
